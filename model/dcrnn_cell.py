    ...
    def _gconv(self, inputs, state, output_size, bias_start=0.0):
        ...
        with tf.variable_scope(scope):
            if self._max_diffusion_step == 0:
                ...
                #for support in self._supports: # end the for loop as len(self._supports) = 1
                x1 = tf.sparse_tensor_dense_matmul(self._adj_mx, x0)
                x = self._concat(x, x1)

                for k in range(2, self._max_diffusion_step + 1):
                    x2 = 2 * tf.sparse_tensor_dense_matmul(self._adj_mx, x1) - x0
                    x = self._concat(x, x2)
                    x1, x0 = x2, x1
